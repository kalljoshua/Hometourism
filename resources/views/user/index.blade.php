...
@section('title')
    <title>Find affordable place to stay in on your next trip with Shatsi Host Homes</title>
@endsection
@section('content')
        ...
        <section class="home-special-offers animatedParent clearfix">
            <div class="container">
                <header class="section-header header-with-nav clearfix">
                    <h3 class="title pull-left animated growIn">FEATURED HOSTS</h3>
                    ...
                    @foreach($featured_services as $featured)
                        <article class="tour-post">
                            ...
                            <header class="tour-post-header clearfix">
                                <span class="tour-price pull-left">$ {{number_format($featured->price)}}</span>
    ...
    @if(sizeof($most_viewed_services)>0)
        <section class="home-special-offers animatedParent clearfix">
            <div class="container">
                <header class="section-header header-with-nav clearfix">
                    <h3 class="title pull-left animated growIn">MOST VIEWED HOSTS</h3>
                    <a class="pull-right animated growIn" href="{{route('services.all')}}">see more offers</a>
                </header>
                <div class="tour-carousel animated flipInX clearfix">
                    @foreach($most_viewed_services as $most_viewed)
                        <article class="tour-post">
                            <i class="circle-icon"></i>
                            <header class="tour-post-header clearfix">
                                <span class="tour-price pull-left">$ {{number_format($most_viewed->price)}}</span>
                                <span class="tour-price-off-h pull-right">
                         @for ($k=1; $k <= 5 ; $k++)
                                        <span data-title="Average Rate: 5 / 5"
                                              class="bottom-ratings tip">
    ...
    @if(sizeof($recent_services)>0)
        <section class="home-special-offers animatedParent clearfix">
            <div class="container">
                <header class="section-header header-with-nav clearfix">
                    <h3 class="title pull-left animated growIn">RECENTLY ADDED HOSTS</h3>
                    <a class="pull-right animated growIn" href="{{route('services.all')}}">see more offers</a>
                </header>
                <div class="tour-carousel animated flipInX clearfix">
                    @foreach($recent_services as $recent)
                        <article class="tour-post">
                            <i class="circle-icon"></i>
                            <header class="tour-post-header clearfix">
                                <span class="tour-price pull-left">$ {{number_format($recent->price)}}</span>
                                <span class="tour-price-off-h pull-right">
                         @for ($k=1; $k <= 5 ; $k++)
                                        <span data-title="Average Rate: 5 / 5"
                                              class="bottom-ratings tip">
    ...
    <section class="home-tour-type animatedParent clearfix">
        <div class="container">
            <header class="text-center">
                <h3 class="title-with-separator animated growIn">Explore Hosts by type</h3>
            </header>
            <div class="row">
                @foreach(App\Type::all() as $type)
    ...
    <section class="weather-location-map animatedParent clearfix">
        <div class="container">
            <header class="text-center">
                <h3 class="title-with-separator animated growIn">Weather & Location</h3>
                <p style="font-size: medium; margin-top: -52px;margin-bottom: 12px">Select a location to see the weather forecast.</p>
            </header>
    ...
    @endif
            ...
            @if(sizeof($populars)>0)
                <section class="home-special-offers animatedParent clearfix">
                    <div class="container">
                        <header class="text-center animatedParent">
                            <h3 class="title-with-separator animated growIn">Popular Destinations</h3>
                        </header>
                        <div class="tour-carousel animated flipInX clearfix">
                            @foreach($populars as $most_viewed)
                                <article class="tour-post">
                                    <div class="tour-contents clearfix">
                                        <a class="more-details u-url" href="/popular-destination/{{$most_viewed->slug}}">
                                        <figure class="tour-feature-img">
                                            <img src="/images/services/our_location_770x370/{{$most_viewed->image}}" alt="Image"/>
                                        </figure>
                                            <h5 class="entry-title p-name">{{$most_viewed->title}}</h5></a>
                                        <span style="color: #FDC600">views ({{$most_viewed->views}})</span>
                                        <a class="more-details u-url" href="/popular-destination/{{$most_viewed->slug}}">
                                            MORE DETAILS <i class="fa fa-angle-right red"></i></a>
                                    </div>
                                </article>
                            @endforeach
                        </div>
